  ...
  function resolveNs(): string | undefined {
    try {
      ...
      if (raw) {
        ...
        const ns =
          u?.tenant?.k8s_namespace || u?.tenant?.ns || u?.k8s_namespace || u?.ns;
              ...
              <table className="w-full text-sm text-white/90">
                ...
                <tbody>
                  {items.map((it) => {
                    ...
                    return (
                      <tr key={`${ns}/${it.name}`} className="border-t border-white/10">
                        <td className="px-4 py-3">{ns}</td>
                        <td className="px-4 py-3 text-cyan-300">{it.name}</td>

                        <td className="px-4 py-3 font-mono truncate max-w-[240px]" title={it.image}>
                          {it.image}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2 items-center justify-center">
                            ...
                            <input
                              ...
                              className="w-20 text-center bg-black/30 border border-cyan-500/20 rounded-lg"
                            />
                            ...
                            <button
                              ...
                              disabled={isBusy}
                              className="px-4 py-1.5 rounded-lg bg-cyan-600 hover:bg-cyan-500"
                            >
                              {isBusy ? <Spinner /> : "Scale"}
                            </button>
                            ...
                            <button
                              onClick={() => {
                                ...
                                const uid =
                                  role === "client" ? "client-dashboard" : "4XuMd2liz";
                                const slug =
                                  role === "client"
                              ...
                              className="px-4 py-1.5 rounded-lg border border-cyan-500/20 text-cyan-300 hover:bg-cyan-500/10"
                            >
                              Monitor
                            </button>
