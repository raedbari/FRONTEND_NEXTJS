...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const isAuthPage = pathname.startsWith("/auth"); // 🔍 تحقق إن كانت الصفحة داخل auth

  return (
    <html lang="en">
      <body className="bg-[#050b14] text-white">
        {/* ✅ Navbar تظهر فقط خارج auth */}
        {!isAuthPage && (
        ...
        {/* ✅ إذا لم يكن auth نضيف مسافة للـ Navbar */}
        <main className={!isAuthPage ? "pt-[88px] px-6" : "px-6"}>
          {children}
        </main>
      </body>
    </html>
