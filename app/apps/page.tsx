...
import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import { listApps } from "@/lib/monitorClient"; // ✅ المصدر الوحيد لنداءات المراقبة

// يتوافق مع شكل /monitor/apps
type AppItem = {
  namespace: string;
  app: string;
  image: string;
...
export default function AppsStatusPage() {
  ...
  async function load() {
    try {
      ...
      setErr(e?.message || "Failed to load apps");
    } finally {
      setLoading(false);
    }
  }
        ...
        <div style={{ overflowX: "auto", marginTop: 12 }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {items.map((it) => {
                const ns = it.namespace || "default";
                const desired = it.replicas_desired ?? 0;
                const available = it.replicas_available ?? 0;

                // لا يوجد svc_selector في هذا الـAPI، فالحالة تكون unknown
                const label = "unknown";
                const cls = "bg-zinc-600/30 text-zinc-300";

                return (
                  <tr key={`${ns}/${it.app}`} style={{ borderTop: "1px solid rgba(255,255,255,.06)" }}>
                    <td style={{ padding: 8 }}>{ns}</td>

                    {/* Name */}
                    <td style={{ padding: 8, fontWeight: 700 }}>{it.app}</td>

                    {/* Image */}
                    <td style={{ padding: 8, fontFamily: "monospace" }}>{it.image}</td>
                    <td style={{ padding: 8 }}>{it.tag || "-"}</td>

                    {/* Numbers */}
                    <td style={{ padding: 8 }}>{desired}</td>
                    <td style={{ padding: 8 }}>{available}</td>

                    {/* Traffic badge (غير متوفر من الـAPI الحالي) */}
                    <td style={{ padding: 8 }}>
                      <span className={`px-2 py-1 rounded text-xs ${cls}`} title="traffic status">
                        {label}
                      </span>
                    ...
                    <td style={{ padding: 8, display: "flex", gap: 8 }}>
                      ...
                      <button
                        className="px-2 py-1 text-sm rounded bg-blue-600"
                        onClick={() => router.push(`/monitor/${ns}/${it.app}`)}
                        title="Monitor"
                      >
                        Monitor
                      </button>

                      {/* Scale (مُعطّل مؤقتًا حتى يتوفر API مناسب) */}
                      <button
                        className="px-2 py-1 text-sm rounded bg-zinc-700 opacity-60 cursor-not-allowed"
                        title="Scale API not available in this build"
                        disabled
                      >
                        Scale
                      </button>
