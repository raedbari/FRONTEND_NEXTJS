...
import { bgRollback } from "../apis/bluegreen"; // ← استيراد صحيح
...
export default function RollbackModal({ initial, onClose, afterSubmit }: Props) {
  ...
  async function submit() {
    try {
      setSubmitting(true);
      await bgRollback(name.trim(), ns.trim());   // 👈 استدعاء API الموحّد
      alert("Rollback done ✅");
      afterSubmit?.();
      onClose();
    } catch (e: any) {
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-zinc-900 text-zinc-100 rounded-2xl p-5 w-[420px] shadow-xl">
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button
            disabled={!canSubmit}
            className={`px-3 py-2 rounded-lg ${canSubmit ? "bg-indigo-600" : "bg-zinc-700/60"}`}
            onClick={submit}
          >
            {submitting ? "Working..." : "Rollback"}
          </button>
        </div>
      </div>
    </div>
